...
RSpec.describe "/people", type: :request do
  ...
  let(:valid_attributes) {
    ...
    [
      # build(:fixed_rate_mortgage).to_params
  ...
  let(:invalid_attributes) {
    ...
    # build(:fixed_rate_mortgage).to_params ssn: "9208211"  # bad param
  }
  let(:new_attributes) {
    skip("Add one or more hash of attributes valid for your model")
    # build(:fixed_rate_mortgage_sample).to_params.slice(*%i[last_name first_name])
  }
